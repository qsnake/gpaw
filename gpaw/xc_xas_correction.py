...
class XCCorrection:
    def __init__(self,
                 ...
                 Exc0,
                 fcorehole=None, # occupation of core hole
                 Z=None,         # nuclear charge
        ...
        if fcorehole is not None:
            # (Z - Nv - fcorehole) is the number of core electrons
            # (0.5 * (Z - Nv) - fcorehole) is the number of alpha core electrons
            #  0.5 * (Z - Nv) is the number of electron pf beta core electrons
            self.nc_g[0] = nc *  (0.5 * (Z - Nv) - fcorehole) / (Z - Nv - fcorehole) 
            self.nct_g[0] = nct * (0.5 * (Z - Nv) - fcorehole) / (Z - Nv - fcorehole) 
            self.nc_g[1] = nc * 0.5 * (Z - Nv) / (Z - Nv - fcorehole)
            self.nct_g[1] = nct * 0.5 * (Z - Nv) / (Z - Nv - fcorehole)
        else:
            self.nc_g[0] = nc 
            self.nct_g[0] = nct 
            self.nc_g[1] = nc 
    ...
    def calculate_energy_and_derivatives(self, D_sp, H_sp, a=None):
        ...
        if len(D_sp) == 1:
            ...
            #na_Lg[0] += 0.5 * self.nc_g[0] * sqrt(4 * pi)
            na_Lg[0] += self.nc_g[0] * sqrt(4 * pi)
            nta_Lg = num.dot(Da_Lq, self.nt_qg)
            #nta_Lg[0] += 0.5 * self.nct_g[0] * sqrt(4 * pi)
            nta_Lg[0] += self.nct_g[0] * sqrt(4 * pi)
            dEdDa_p = H_sp[0][:]
            dEdDa_p[:] = 0.0
            Db_p = D_sp[1]
            Db_Lq = dot3(self.B_Lqp, Db_p)
            nb_Lg = num.dot(Db_Lq, self.n_qg)
            #nb_Lg[0] += 0.5 * self.nc_g[1] * sqrt(4 * pi)
            nb_Lg[0] +=  self.nc_g[1] * sqrt(4 * pi)
            ntb_Lg = num.dot(Db_Lq, self.nt_qg)
            #ntb_Lg[0] += 0.5 * self.nct_g[1] * sqrt(4 * pi)
            ntb_Lg[0] +=  self.nct_g[1] * sqrt(4 * pi)
            dEdDb_p = H_sp[1][:]
            dEdDb_p[:] = 0.0
            for w, Y_L in zip(self.weights, self.Y_yL):
    ...
    def GGA(self, D_sp, H_sp):
        ...
        if len(D_sp) == 1:
            ...
            #nat_Lg[0] += 0.5 * self.nct_g[0] * sqrt(4 * pi)
            nat_Lg[0] += self.nct_g[0] * sqrt(4 * pi)
            ...
            #nbt_Lg[0] += 0.5 * self.nct_g[1] * sqrt(4 * pi)
            nbt_Lg[0] += self.nct_g[1] * sqrt(4 * pi)
