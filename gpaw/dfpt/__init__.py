...
equations for the 1) variation of the effective potential, 2) variation of the
wave functions, and 3) the resulting variation of the density. The variation of
the wave-functions can be determined from a set of linear equations, the
so-called Sternheimer equation. Since only their projection onto the unoccupied
state manifold is required to determine the corresponding density variation,
only the occupied wave functions/states of the unperturbed system are
needed. This is in contrast to the standard textbook expression for the
first-order variation of the wave function which involves the full spectrum of
the unpertured system.
