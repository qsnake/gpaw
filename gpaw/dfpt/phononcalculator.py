...
class PhononCalculator:
    ...
    def dynamical_matrix(self, eps,
        ...
        # Integrate first-order Hartree potential with ghat derivative
        dghat_aLvv = dict([
            (atom.index, np.zeros((ghat.get_function_count(atom.index), 3, 3)))
            for atom in self.atoms ])
        # Integrate first-order density variation with vbar derivative
        dvbar_avv = dict([ (atom.index, np.zeros((3,3)))
                           for atom in self.atoms ])
        ...
        for atom in self.atoms:
            ...
            for v in [0,1,2]:
                ...
                if use_dfpt:
                    ...
                    self.perturbation.calculate_dP_aniv()
                    self.perturbation.calculate_d2P_anivv()
                ...
                for atom in self.atoms:
                    a = atom.index
                    dghat_aLvv[a][:,v,:] += dghat_aLv[a]
                    dghat_aLvv[a][:,v,:] += dghat2_aLv[a]
                    dvbar_avv[a][v] += dvbar_av[a]
