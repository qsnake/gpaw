...
def test(complex_type):
    ...
    A[0:M,:] = 0.0*np.eye(M,N,0)
    A[0:M,:] = A[0:M,:]+ 0.1*np.eye(M,N,+M*0-1)*epsilon # diag +1
    A[M:2*M,:] = 1.0*np.eye(M,N,+M)
    A[M:2*M,:] = A[M:2*M,:] + 0.1*np.eye(M,N,+M*1-1)*epsilon # diag +1
    A[2*M:3*M,:] = 2.0*np.eye(M,N,+M*2)
    A[2*M:3*M,:] = A[2*M:3*M,:] + 0.1*np.eye(M,N,+M*2-1)*epsilon # diag +1
    A[3*M:4*M,:] = 3.0*np.eye(M,N,+M*3)
    A[3*M:4*M,:] = A[3*M:4*M,:]+ 0.1*np.eye(M,N,+M*3-1)*epsilon # diag +1
    if debug and world.rank == 0:
        print "A = ", A
    ...
    S[0:M,:] = S[0:M,:]+ 0.2*np.eye(M,N,+M*0-1)*epsilon # diag +1
    S[M:2*M,:] = 1.0*np.eye(M,N,+M)
    S[M:2*M,:] = S[M:2*M,:] + 0.2*np.eye(M,N,+M*1-1)*epsilon # diag +1
    S[2*M:3*M,:] = 1.0*np.eye(M,N,+M*2)
    S[2*M:3*M,:] = S[2*M:3*M,:] + 0.2*np.eye(M,N,+M*2-1)*epsilon # diag +1 
    S[3*M:4*M,:] = 1.0*np.eye(M,N,+M*3)
    S[3*M:4*M,:] = S[3*M:4*M,:] + 0.2*np.eye(M,N,+M*3-1)*epsilon # diag +1
    if debug and world.rank == 0:
        print "S = ", S
    ...
    A_nm = A_nm[0:M,:] + 0.1*np.eye(M,N,+M*world.rank-1)*epsilon
    A_nm = A_nm.copy("Fortran") # Fortran order required for ScaLAPACK
    S_nm = np.eye(M,N,+M*world.rank)
    S_nm = S_nm[0:M,:] + 0.2*np.eye(M,N,+M*world.rank-1)*epsilon
    ...
    desc1 = blacs_create(world,N,N,B,1,M,N)
