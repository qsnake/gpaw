...
def condYdYdtheta(l1, m1, l2, m2, lquad=11, out=False):
    ...
    out |= (l1-l2)%2 == 1 and abs(m1-m2)%4 == 3 and m1*m2 < 0 #~75%, fpos~0%
    out |= (l1+l2)%2 == 1 and m1==0 and m2%2 == 1 #~72%, fpos~0.3%

    # Works well if lmax>=6 as lquad=11 and 11//2=5, otherwise irrelevant
    out |= l1+l2 >= lquad and (l1-l2)%2 == 1 and abs(m1-m2)%2 == 1 #~89%, fpos~0.1%
    out |= l1+l2 >= lquad and (l1-l2)%2 == 1 and abs(m1-m2)%4 == 0 # ~79%, fpos~0%
    return out

def condYdYdphi(l1, m1, l2, m2, lquad=11, out=False):
    """Predict whether this combination requires a higher-order quadrature."""
    # Address minor issues when l1+l2 < lquad
    out |= (l1-l2)%2 == 1 and abs(m1-m2)%4 == 3 and m1*m2 < 0 #~89%, fpos~0%
    out |= (l1+l2)%2 == 1 and m1==0 and m2%2 == 1 #~86%, fpos~0.4%

    # Works well if lmax>=6 as lquad=11 and 11//2=5, otherwise irrelevant
    out |= l1+l2 > lquad and (l1-l2)%2 == 1 and abs(m1-m2)%2 == 1 and m2 != 0 #~97%, fpos~0.1%
    return out
...
class UTSphereParallelSetup(TestCase):
    ...
    def test_multipole_expansion(self):
        ...
        # Test multipole expansion in complex spherical harmonics
        R = 1.0
        npts = 1000
        tol = 1e-9
    ...
    def test_relation_real(self):
        ...
        for l,m in lmiter(9, comm=world):
            if m == 0:
                ...
                s_L = (-1)**m*(_Y(l,m,theta_L,phi_L)+1j*_Y(l,-m,theta_L,phi_L))/2**0.5
            else:
                s_L = (_Y(l,abs(m),theta_L,phi_L)-1j*_Y(l,-abs(m),theta_L,phi_L))/2**0.5
