...
def initialize_parameters(code, width, h):
    ...
    if code == 'gpaw':
        ...
        parameters['gmaxvr'] = 13 # default 12
        parameters['rgkmax'] = 6.0 # default 7
        #parameters['gmaxvr'] = 14 # default 12
        #parameters['rgkmax'] = 6.5 # default 7
...
def run_molecule(xc, code):

    parameters = initialize_parameters(code, 0.05, h)
    parameters['xc'] = xc
...
if __name__ == '__main__':
    ...
    assert opt.element in element_choices, opt.element+' not in '+str(element_choices)
    assert opt.xc in xc_choices, opt.xc+' not in '+str(xc_choices)
    if opt.mode == 'molecule':
        assert opt.element == 'None', 'element in molecule: not implemented yet'
    ...
    if opt.mode == 'molecule':
        run_molecule(opt.xc, opt.code)
    elif opt.mode == 'slab':
        run_slab(str(opt.element), opt.xc, opt.code)
