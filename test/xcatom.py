...
for xc in ['LDA', 'PBE']:
    ...
    E1 = s.xc.calculate_energy_and_derivatives([D_p], [H_p])
    dD_p = x * ra.random(np)
    D_p += dD_p
    dE = num.dot(H_p, dD_p) / x
    E2 = s.xc.calculate_energy_and_derivatives([D_p], [H_p])
    equal(dE, (E2 - E1) / x, 0.003)

    d = Setup('N', xcfunc, nspins=2)
    E2s = s.xc.calculate_energy_and_derivatives([0.5 * D_p, 0.5 * D_p],
                                             [H_p, H_p])
    ...
    E1 = s.xc.calculate_energy_and_derivatives(D_sp, H_sp)
    dD_sp = x * ra.random((2, np))
    D_sp += dD_sp
    dE = num.dot(H_sp.flat, dD_sp.flat) / x
    E2 = s.xc.calculate_energy_and_derivatives(D_sp, H_sp)
