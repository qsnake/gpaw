...
\title{\textbf{The Projector Augmented-wave Method}}
\author{Carsten Rostgaard}
%\date{October 19, 2009}
\date{Available online at \url{http://arxiv.org/abs/0910.1921}.}
...
expansion of $n(\br)$ according to \eqref{eq: PAW density}, one would
get:
%
\begin{align*}
...
charge neutral (see eq. \eqref{eq: rhot charge neutral}).
...
needed, and for GW calculations (see e.g.  \cite{Rostgaard2009}),
where all elements are needed.

Introducing the PAW transformation in \eqref{eq:coulomb-matrix}, the
...
\eqref{eq:coulomb-matrix} would, due to the non-local nature of the
...
to the Coulomb energy \eqref{eq:coulom tensor 2}. The only
...
The formally exact partitioning \eqref{eq:coulomb-matrix-partition}
...
PAW as in equation \eqref{eq:coulomb-matrix-partition}. Terms where
either index refers to a core orbital can be reduce to
...
equations by redefining equations \eqref{eq:coulom tensor 2},
\eqref{eq:coulom tensor 1}, and \eqref{eq:coulom tensor 0}
...
via the eigenvalues as done in equation \eqref{eq:kinetic-energy}. The
additional term (besides those related to redefining \eqref{eq:coulom
  tensor 0}--\eqref{eq:coulom tensor 2})
\begin{equation}
...
\end{equation}
should be added to the right hand side of \eqref{eq:kinetic-energy} on
inclusion of exact exchange.

In a similar fashion, the compensation charges leads to an additional
force contribution in equation \eqref{eq: force no exx} given by
...
equation \eqref{eq: nonlocal exchange} and $\hat{v}^\text{L}_x$ is the
local version in \eqref{eq: local exchange}.
