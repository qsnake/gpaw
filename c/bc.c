    ...
    if (i == 0)
      {
        ...
        if (real)
          bmgs_paste(aa1 + m * ng, bc->size1, aa2 + m * ng2, bc->size2, bc->sendstart[0][0]);
        else
          bmgs_pastez((const double_complex*)(aa1 + m * ng), bc->size1,
                    (double_complex*)(aa2 + m * ng2), bc->size2, bc->sendstart[0][0]);
      }
      ...
      if (p >= 0)
        {
          if (bc->rjoin[i])
            {
              if (d == 0)
                MPI_Irecv(rbuff, (bc->nrecv[i][0] + bc->nrecv[i][1]) * nin, MPI_DOUBLE, p,
                          10 * thd + 1000 * i + 100000,
                          bc->comm, &recvreq[0]);
            }
          else
          {
            MPI_Irecv(rbuff, bc->nrecv[i][d] * nin, MPI_DOUBLE, p, d + 10 * thd + 1000 * i,
                      bc->comm, &recvreq[d]);
          }
        }
    }
          ...
          if (bc->sjoin[i])
            {
              for (int m = 0; m < nin; m++)
                if (real)
                  bmgs_cut(aa2 + m * ng2, bc->size2, start, sbuf + m * (bc->nsend[i][0] + bc->nsend[i][1]), size);
                else
                  bmgs_cutmz((const double_complex*)(aa2 + m * ng2), bc->size2, start,
                             (double_complex*)(sbuf + m * (bc->nsend[i][0] + bc->nsend[i][1])), size, phases[d]);

              if (d == 1)
                {
                  MPI_Isend(sbuf0, (bc->nsend[i][0] + bc->nsend[i][1]) * nin, MPI_DOUBLE, p,
                            10 * thd + 1000 * i + 100000,
                            bc->comm, &sendreq[0]);
                }
            }
          else
            {
              for (int m = 0; m < nin; m++)
                if (real)
                  bmgs_cut(aa2 + m * ng2, bc->size2, start, sbuf + m * bc->nsend[i][d], size);
                else
                  bmgs_cutmz((const double_complex*)(aa2 + m * ng2), bc->size2, start,
                             (double_complex*)(sbuf + m * bc->nsend[i][d]), size, phases[d]);

              MPI_Isend(sbuf0, bc->nsend[i][d] * nin, MPI_DOUBLE, p,
        ...
        if (bc->rjoin[i])
          {
            ...
            for (int m = 0; m < nin; m++)
              if (real)
                bmgs_paste(rbuf + m * (bc->nrecv[i][0] + bc->nrecv[i][1]), bc->recvsize[i][d],
                     a2 + m * ng2, bc->size2, bc->recvstart[i][d]);
              else
                bmgs_pastez((const double_complex*)(rbuf + m * (bc->nrecv[i][0] + bc->nrecv[i][1])), bc->recvsize[i][d],
                      (double_complex*)(a2 + m * ng2), bc->size2, bc->recvstart[i][d]);
          }
        else
          {
            ...
            for (int m = 0; m < nin; m++)
              if (real)
                ...
                bmgs_pastez((const double_complex*)(rbuf + m * bc->nrecv[i][d]), bc->recvsize[i][d],
                      (double_complex*)(a2 + m * ng2), bc->size2, bc->recvstart[i][d]);
          }
