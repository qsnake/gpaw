...
static MPI_Datatype get_mpi_datatype(PyObject *a)
{
  ...
  else if (PyArray_ISFLOAT(a) || PyArray_ISCOMPLEX(a))
    switch(n)
      {
      case sizeof(float):
	return MPI_FLOAT;
  ...
  // If we reach this point either none of the ifs or none of the
  // cases worked out.
