                                 ...
                                 PyObject *args)
{
  ...
  #else
    MPI_Request *recvreq = self->recvreq;
    MPI_Request *sendreq = self->sendreq;
  #endif
  ...
  for (int n = 0; n < nin; n++)
    {
      ...
      double* out = outt + n * ng;
        ...
        int thd = 0;
      #endif
      double* sendbuf = self->sendbuf + thd * self->bc->maxsend;
      double* recvbuf = self->recvbuf + thd * self->bc->maxrecv;
      double* buf = self->buf + thd * buf_offset;

      for (int i = 0; i < 3; i++)
        {
          bc_unpack1(bc, in, buf, i,
               recvreq + 2 * thd, sendreq + 2 * thd,
               recvbuf, sendbuf, ph + 2 * i);
          bc_unpack2(bc, buf, i,
               recvreq + 2 * thd, sendreq + 2 * thd, recvbuf);
        }
      if (real)
...
PyObject * NewOperatorObject(PyObject *obj, PyObject *args)
{
  ...
  self->sendbuf = GPAW_MALLOC(double, self->bc->maxsend * omp_get_max_threads());
  self->recvbuf = GPAW_MALLOC(double, self->bc->maxrecv * omp_get_max_threads());
#endif
