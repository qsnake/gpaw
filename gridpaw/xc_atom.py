...
class XCAtom1:
    def __init__(self, xc, w_j, wt_j, nc, nct, rgd, jl, lmax, Exc0):
        ...
        np = ni * (ni + 1) / 2
        nq = nj * (nj + 1) / 2
        self.A_qp = num.zeros((nq, np), num.Float)
        p = 0
        i1 = 0
        for j1, l1, L1 in jlL:
            for j2, l2, L2 in jlL[i1:]:
                if j1 < j2:
                    ...
                    q = j1 + j2 * nj - j2 * (j2 + 1) / 2
                if L1 == L2:
                    self.A_qp[q, p] = 1.0 / (4.0 * pi)
                p += 1
            i1 += 1
        self.dv = rgd.dv_g
        self.n_q = num.zeros((nq, ng), num.Float)
        self.nt_q = num.zeros((nq, ng), num.Float)
        q = 0
        for j1, l1 in jl:
            for j2, l2 in jl[j1:]:
                rl1l2 = rgd.r_g**(l1 + l2)
                self.n_q[q] = rl1l2 * w_j[j1] * w_j[j2]
                self.nt_q[q] = rl1l2 * wt_j[j1] * wt_j[j2]
                q += 1
       
    def CalculateEnergyAndDerivatives(self, D_sp, H_sp):
        if len(D_sp) == 1:
            D_p = D_sp[0]

            D_q = num.dot(self.A_qp, D_p)
            n = self.nc + num.dot(D_q, self.n_q)
            vxc = num.zeros(self.ng, num.Float) 
            E = self.xc.GetEnergyAndPotential(n, vxc)
            dEdD_q = num.dot(self.n_q, vxc * self.dv)
            n[:] = self.nct + num.dot(D_q, self.nt_q)
            vxc[:] = 0.0
            E -= self.xc.GetEnergyAndPotential(n, vxc)
            dEdD_q -= num.dot(self.nt_q, vxc * self.dv)
            dEdD_p = num.dot(dEdD_q, self.A_qp)

            H_sp[0][:] = dEdD_p
        else:
            Da_q = num.dot(self.A_qp, D_sp[0])
            Db_q = num.dot(self.A_qp, D_sp[1])
            na_g = 0.5 * self.nc + num.dot(Da_q, self.n_q)
            nb_g = 0.5 * self.nc + num.dot(Db_q, self.n_q)
            vxca = num.zeros(self.ng, num.Float) 
            vxcb = num.zeros(self.ng, num.Float) 
            E = self.xc.GetEnergyAndPotential(na_g, vxca, nb_g, vxcb)
            dEdDa_q = num.dot(self.n_q, vxca * self.dv)
            dEdDb_q = num.dot(self.n_q, vxcb * self.dv)
            na_g[:] = 0.5 * self.nct + num.dot(Da_q, self.nt_q)
            nb_g[:] = 0.5 * self.nct + num.dot(Db_q, self.nt_q)
            vxca[:] = 0.0
            vxcb[:] = 0.0
            E -= self.xc.GetEnergyAndPotential(na_g, vxca, nb_g, vxcb)
            dEdDa_q -= num.dot(self.nt_q, vxca * self.dv)
            dEdDb_q -= num.dot(self.nt_q, vxcb * self.dv)
            dEdDa_p = num.dot(dEdDa_q, self.A_qp)
            dEdDb_p = num.dot(dEdDb_q, self.A_qp)
...
class XCAtom2:
    def __init__(self, xc, w_j, wt_j, nc, nct, rgd, jl, lmax, Exc0):
        assert not xc.GetXCFunctional().gga, "Can't do GGA!"
        self.nc = nc
        self.nct = nct
        self.xc = xc
        self.Exc0 = Exc0
        if lmax == 0:
            self.weights = [1.0]
            YY = num.array([[1.0 / (4.0 * pi)]])
        else:
            self.weights = Sphere.weights
            YY = Sphere.YY
        ny = len(YY)
        jlL = []
        for j, l in jl:
        ...
        np = ni * (ni + 1) / 2
        nq = nj * (nj + 1) / 2
        self.YY_yqp = num.zeros((ny, nq, np), num.Float)
        p = 0
        i1 = 0
        for j1, l1, L1 in jlL:
            for j2, l2, L2 in jlL[i1:]:
                if j1 < j2:
                    ...
                    q = j1 + j2 * nj - j2 * (j2 + 1) / 2
                if L1 < L2:
                    LL = L2 + L1 * 9 - L1 * (L1 + 1) / 2
                else:
                    LL = L1 + L2 * 9 - L2 * (L2 + 1) / 2
                self.YY_yqp[:, q, p] += YY[:, LL]
                p += 1
            i1 += 1
        self.dv = rgd.dv_g
        self.n_q = num.zeros((nq, ng), num.Float)
    ...
    def CalculateEnergyAndDerivatives(self, D_sp, H_sp):
        ...
        if len(D_sp) == 1:
            ...
            for D_q in D_yq:
                ...
                n = self.nc + num.dot(D_q, self.n_q)
                vxc = num.zeros(self.ng, num.Float) 
                E += self.xc.GetEnergyAndPotential(n, vxc) * w
                dEdD_q = num.dot(self.n_q, vxc * self.dv)
                n[:] = self.nct + num.dot(D_q, self.nt_q)
                vxc[:] = 0.0
                E -= self.xc.GetEnergyAndPotential(n, vxc) * w
                dEdD_q -= num.dot(self.nt_q, vxc * self.dv)
                dEdD_p += num.dot(dEdD_q, self.YY_yqp[y]) * w
                y += 1

        else:
            Da_yq = num.dot(self.YY_yqp, D_sp[0])
            Db_yq = num.dot(self.YY_yqp, D_sp[1])
            dEdDa_p = H_sp[0][:]
            dEdDb_p = H_sp[1][:]
            dEdDa_p[:] = 0.0
            dEdDb_p[:] = 0.0
            y = 0
            for Da_q, Db_q in zip(Da_yq, Db_yq):
                w = self.weights[y]
                na_g = 0.5 * self.nc + num.dot(Da_q, self.n_q)
                nb_g = 0.5 * self.nc + num.dot(Db_q, self.n_q)
                vxca = num.zeros(self.ng, num.Float) 
                vxcb = num.zeros(self.ng, num.Float) 
                E += self.xc.GetEnergyAndPotential(na_g, vxca, nb_g, vxcb) * w
                dEdDa_q = num.dot(self.n_q, vxca * self.dv)
                dEdDb_q = num.dot(self.n_q, vxcb * self.dv)
                na_g[:] = 0.5 * self.nct + num.dot(Da_q, self.nt_q)
                nb_g[:] = 0.5 * self.nct + num.dot(Db_q, self.nt_q)
                ...
                dEdDa_q -= num.dot(self.nt_q, vxca * self.dv)
                dEdDb_q -= num.dot(self.nt_q, vxcb * self.dv)
                dEdDa_p += num.dot(dEdDa_q, self.YY_yqp[y]) * w
                dEdDb_p += num.dot(dEdDb_q, self.YY_yqp[y]) * w
                y += 1
